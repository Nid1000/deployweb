  ...
  const columns = useMemo<ColumnDef<Comprobante>[]>(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const pdf = assetUrl(a.pdf);
        const xml = assetUrl(a.xml);
        const img = assetUrl(a.img);
        return (
          <div className="flex items-center justify-end gap-2">
            {pdf ? (
              <a className={buttonClasses({ variant: "outline-primary", size: "sm" })} href={pdf} target="_blank" rel="noreferrer">PDF</a>
            ) : null}
            {xml ? (
              <a className={buttonClasses({ variant: "outline-secondary", size: "sm" })} href={xml} target="_blank" rel="noreferrer">XML</a>
            ) : null}
            {img ? (
              <a className={buttonClasses({ variant: "outline-secondary", size: "sm" })} href={img} target="_blank" rel="noreferrer">IMG</a>
            ) : null}
          </div>
        );
      },
