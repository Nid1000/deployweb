...
export default function OrdersPage() {
  const { isAuthenticated, loading: authLoading } = useAuth() as unknown as {
  ...
  }, [authLoading, isAuthenticated, pagina, limite]);
  ...
  const estadosDisponibles = useMemo(() => {
    return Array.from(
      new Set(pedidos.map((p) => String(p.estado ?? "").trim()).filter(Boolean))
    );
  }, [pedidos]);
  const pedidosFiltrados = useMemo(() => {
    ...
    if (busqueda) {
      ...
      list = list.filter((p) => {
        const estadoVal = String(p.estado ?? "").toLowerCase();
        return String(p.id).includes(q) || estadoVal.includes(q);
      });
    }
    if (estado) {
      list = list.filter((p) => String(p.estado ?? "") === estado);
    }
    if (desde) {
              ...
              {loading ? (
                ...
                <div className="space-y-4">
                      ...
                      <div key={p.id} className="border rounded p-4 flex items-center justify-between">
                        <div>
                          ...
                          <div className="text-sm text-gray-600">
                            {formatDateTime(p.created_at || p.fecha_pedido)}
                          </div>
                  ...
                  {!loading && pedidosFiltrados.length === 0 && (
                    <div className="text-gray-600 mt-3">No hay pedidos disponibles.</div>
                  )}
                </div>
              )}
                  ...
                  {comprobantesFiltrados.map((c, idx) => (
                    <div key={idx} className="border rounded p-3 flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-sm text-gray-600">
                          Emitido: {formatDateTime(c.created_at)}
                        </div>
                      </div>
                        ...
                        {c.archivos?.pdf && (
                          <>
                            <button
                              className="px-3 py-1.5 rounded border hover:bg-gray-50"
                              onClick={() => {
                                setPdfModal({ open: true, src: assetUrl(c.archivos!.pdf), titulo: `${c.tipo} ${c.serie}-${c.numero}` });
                              }}
                            >
                              Ver PDF
                            </button>
                            <a
                              href={assetUrl(c.archivos.pdf)}
                              download
                              target="_blank"
                              rel="noreferrer"
                              className="px-3 py-1.5 rounded border hover:bg-gray-50"
                        ...
                        {c.archivos?.img && (
                          <button
                            ...
                            onClick={(e) => {
                              ...
                              setImgModal({ open: true, src: getImageSrc(assetUrl(c.archivos!.img) || "", { width: 1024 }), titulo: `${c.tipo} ${c.serie}-${c.numero}` });
                            }}
                          >
                            Ver imagen
                          </button>
                    ...
                    <motion.div role="dialog" aria-modal="true" aria-label="PDF del comprobante" className="bg-white rounded-lg shadow-xl max-w-4xl w-full overflow-hidden" initial={{ scale: 0.95, y: 8 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: -8 }}>
                      ...
                      <div className="p-3">
                        {pdfModal.src ? (
                          ...
                          <div className="text-sm text-gray-600">No hay PDF disponible.</div>
                        )}
                        <div className="mt-3 flex items-center gap-2">
                          {pdfModal.src && (
                            <>
                              <a href={pdfModal.src} target="_blank" rel="noreferrer" className="px-3 py-1.5 rounded border hover:bg-gray-50">Abrir en nueva pestaña</a>
                              <a href={pdfModal.src} download target="_blank" rel="noreferrer" className="px-3 py-1.5 rounded border hover:bg-gray-50">Descargar PDF</a>
                            </>
                          )}
                        </div>
                      </div>
